...
class UserSpecific(models.Model):
    ...
    fields = fields.One2many(
        "user.fields", "fields_list", "Fields Information"
    )

    @api.model
    def ks_duplicate_record(self, ids, ks_model_name):
        for id in ids:
            self.env[ks_model_name].sudo().browse(id).copy()
    ...
    def check_user_exists(self, model_name, uid, ks_action_id):
        ...
        if user_exists:
            return user_exists.fields.read(
                ['ksShowField', 'field_name', 'ks_invisible', 'ks_columns_name','ks_width']) + user_exists.read(['ks_table_width'])
        else:
            return False
...
class Userfields(models.Model):
    ...
    ks_width = fields.Float(String = "Field Width")
...
class KsHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        rec['ks_list_view_field_mode'] = self.env['ir.config_parameter'].sudo().get_param('ks_list_view_field_mode')
