...
class EmployeeTransfer(models.Model):
    ...
    date = fields.Date(string='Ngày điều chuyển', default=fields.Date.today())
    branch = fields.Many2one('transfer.company', string='Cty/Chi nhánh chuyển nhượng',  copy=False,required=True)
    ...
    company_id = fields.Many2one('res.company', string='CTy/Chi nhánh',
                                 related='employee_id.company_id')
    note = fields.Text(string='Ghi chú')
    transferred = fields.Boolean(string='Transferred', copy=False, default=False, compute='_get_transferred')
    responsible = fields.Many2one('hr.employee', string='Chịu trách nhiệm', default=_default_employee, readonly=True)
