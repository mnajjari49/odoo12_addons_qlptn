...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        if employee:
            ...
            mobile_phone = employee[0]['mobile_phone']
            if employee[0]['country_of_birth']:
                country = employee[0]['country_of_birth']
            ...
            if employee[0]['joining_date']:
                joining_date = datetime.strftime(employee[0]['joining_date'], '%d-%m-%Y')
                diff = relativedelta(datetime.today(), employee[0]['joining_date'])
                years = diff.years
                months = diff.months
                days = diff.days
                experience = '{} năm {} tháng {} ngày'.format(years, months, days)
            else:
                experience = False
                joining_date = False
            if employee:
