...
class Employee(models.Model):
    ...
    appraisal_count = fields.Integer(compute='_compute_appraisal_count', string="Đánh giá", type='integer')

    @api.multi
    def _compute_appraisal_count(self):
        AccountAnalyticAccount = self.env['survey.user_input']
        for partner in self:
            partner.appraisal_count = AccountAnalyticAccount.search_count(
                [('state', 'in', ('done', 'skip')), ('employee_id', '=', partner.id)])
...
class HrAppraisalForm(models.Model):
    ...
    emp_id = fields.Many2one('hr.employee', string="Nhân viên", required=True)
    appraisal_deadline = fields.Date(string="Hạn chót đánh giá", required=True)
    final_interview = fields.Date(string="Khảo sát cuối cùng", help="After sending survey link,you can"
    ...
    hr_manager = fields.Boolean(string="Quản lý", default=False)
    hr_emp = fields.Boolean(string="Nhân viên", default=False)
    hr_colloborator = fields.Boolean(string="Cộng tác viên", default=False)
    hr_colleague = fields.Boolean(string="Đồng nghiệp", default=False)
    hr_manager_id = fields.Many2many('hr.employee', 'manager_appraisal_rel', string="Chọn người xem lại đánh giá")
    hr_colleague_id = fields.Many2many('hr.employee', 'colleagues_appraisal_rel',
                                       string="Chọn người đánh giá")
    hr_colloborator_id = fields.Many2many('hr.employee', 'colloborators_appraisal_rel',
                                          string="Chọn người đánh giá")
    manager_survey_id = fields.Many2one('survey.survey', string="Chọn mẫu đánh giá")
    emp_survey_id = fields.Many2one('survey.survey', string="Chọn người đánh giá")
    colloborator_survey_id = fields.Many2one('survey.survey', string="Chọn mẫu đánh giá")
    colleague_survey_id = fields.Many2one('survey.survey', string="Chọn mẫu đánh giá")
    response_id = fields.Many2one('survey.user_input', "Response", ondelete="set null", oldname="response")
    final_evaluation = fields.Text(string="Đánh giá cuối cùng")
    app_period_from = fields.Datetime("From", required=True, readonly=True, default=fields.Datetime.now())
    tot_comp_survey = fields.Integer(string="Đếm câu trả lời", compute="_compute_completed_survey")
    tot_sent_survey = fields.Integer(string="Đếm câu hỏi đã gửi")
    ...
    def fetch_appraisal_reviewer(self):
        ...
        if self.hr_manager and self.hr_manager_id and self.manager_survey_id:
            appraisal_reviewers.append((self.hr_manager_id, self.manager_survey_id, 'manager'))
        if self.hr_emp and self.emp_survey_id:
            appraisal_reviewers.append((self.emp_id, self.emp_survey_id, 'employee'))
        if self.hr_colloborator and self.hr_colloborator_id and self.colloborator_survey_id:
            appraisal_reviewers.append((self.hr_colloborator_id, self.colloborator_survey_id, 'colloborator'))
        if self.hr_colleague and self.hr_colleague_id and self.colleague_survey_id:
            appraisal_reviewers.append((self.hr_colleague_id, self.colleague_survey_id, 'colleague'))
    ...
    def action_start_appraisal(self):
        ...
        for appraisal_reviewers, survey_id, tmp in appraisal_reviewers_list:
            for reviewers in appraisal_reviewers:
                response = self.env['survey.user_input'].create(
                    {'survey_id': survey_id.id, 'partner_id': reviewers.user_id.partner_id.id,
                     ...
                     'email': reviewers.user_id.email,  'employee_id': reviewers.id})
                token = response.token
                if token:
                    url = survey_id.public_url
                    url = url + '/' + token
                    if tmp == 'manager':
                        mail_content = "Dear " + reviewers.name + "," \
                                       + "<br>Anh/Chị vui lòng điền vào bản khảo sát sau đây liên quan đến nhân viên: " + self.emp_id.name + \
                                       "<br>Bấm vào đây để đi đến khảo sát.<br>" + str(url) + \
                                       "<br>Gửi câu trả lời cho phần đánh giá trước : " + str(self.appraisal_deadline) + \
                                       "<br>Trân trọng cám ơn!"
                    elif tmp == 'employee':
                        mail_content = "Dear " + reviewers.name + "," + \
                                       "<br>Anh/Chị vui lòng điền vào bản khảo sát sau đây liên quan đến bạn" \
                                       "<br>Bấm vào đây để đi đến khảo sát.<br>" + str(url) + \
                                       "<br>Gửi câu trả lời cho phần đánh giá trước : " + str(self.appraisal_deadline) + \
                                       "<br>Trân trọng cám ơn!"
